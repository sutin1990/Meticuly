    ...
    public class GridFGMovementController : Controller
    {
        ...
        public IActionResult Print(string RowNumber, string fromdate, string todate)
        {
            ...
            if (RowNumber != null)
            {
                ...
                {
                    var oldipaddress = Log_Select_Print.Where(w => w.opt == "PrintFGMovement" && w.ipaddress == ip);
                    if (oldipaddress.Count()>0)
                    {
                        var pp_update = _context.Log_Select_Print.FirstOrDefault(f => f.opt == "PrintFGMovement" && f.ipaddress == ip);
