    ...
    public class GridWipMovementController : Controller
    {
        ...
        public IActionResult Print( string RowNumber , string fromdate, string todate, string Process, string FCode, string ItemCode, string Model)
        {
            ...
            if (RowNumber != null)
            {
                ...
                {
                    var oldipaddress = Log_Select_Print.Where(w => w.opt == "PrintWipMovement" && w.ipaddress == ip);
                    if (oldipaddress.Count() > 0)
                    {
                        var pp_update = _context.Log_Select_Print.FirstOrDefault(f => f.opt == "PrintWipMovement" && f.ipaddress == ip);
