    ...
    public class GridDCWIPbyProcessHController : Controller
    {
        ...
        public IActionResult Print(string RowNumber,string fromdate, string todate)
        {
            ...
            if (RowNumber != null)
            {
                ...
                {
                    var oldipaddress = Log_Select_Print.Where(w => w.opt == "PrintDCWIPbyProcessHistory" && w.ipaddress == ip);
                    if (oldipaddress.Count() > 0)
                    {
                        var pp_update = _context.Log_Select_Print.FirstOrDefault(f => f.opt == "PrintDCWIPbyProcessHistory" && f.ipaddress == ip);
                    ...
                    else
                    {
                        ...
                        {
                            opt = "PrintDCWIPbyProcessHistory",
                            name = RowNumber,
                            ipaddress = ip
                        };
